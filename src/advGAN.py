...
class Generator(nn.Module):
    def __init__(self,
        ...
        decoder_lis = [
            ...
            nn.InstanceNorm2d(16),
            nn.ReLU(),
            # state size. 16 x 11 x 11 # 16 X 147 X 147
            nn.ConvTranspose2d(16, 8, kernel_size=3, stride=2, padding=0, bias=False),
            nn.InstanceNorm2d(8),
            nn.ReLU(),
            # state size. 8 x 23 x 23 # 8 X 295 X 295
            # nn.ConvTranspose2d(8, image_nc, kernel_size=6, stride=1, padding=0, bias=False),
            nn.ConvTranspose2d(8, image_nc, kernel_size=5, stride=1, padding=0, bias=False),
            nn.Tanh()
            # state size. image_nc x 299 x 299 # 
        ]
...
class AdvGAN(object):
    def __init__(self,checkpoint,device,eps) -> None:
    ...
    def __call__(self, x, y):
        ...
        perturbation = torch.clamp(
            perturbation, -self.eps/255., self.eps/255.)
