...
import math
class GAP(object):
    def __init__(self,model,checkpoint,device,mean,std,mode) -> None:
        self.model=model
        self.mode=mode
        self.mean=mean
        self.std=std
    ...
    def __call__(self, x, y):
        ...
        elif self.mode=='imdep':
            delta_im = self.netG(x)
            delta_im = self.normalize_and_scale(delta_im,'test')
        recons=x+delta_im
        for cii in range(3):
            recons[:,cii,:,:] = recons[:,cii,:,:].clone().clamp(x[:,cii,:,:].min(), x[:,cii,:,:].max())
        recons=recons*self.std+self.mean
        return recons.detach()


    def normalize_and_scale(self,delta_im,mode):
        delta_im = nn.ConstantPad2d((0,-1,-1,0),0)(delta_im) 
        delta_im = delta_im + 1 # now 0..2
        delta_im = delta_im * 0.5 # now 0..1
        delta_im = (delta_im.clone()-self.mean)/self.std
        for ci in range(3):
            l_inf_channel = delta_im[0,ci,:,:].detach().abs().max()
            mag_in_scaled_c = 10/(255.0*self.std[0,ci,0,0])
            delta_im[0,ci,:,:] = delta_im[0,ci,:,:].clone() * np.minimum(1.0, mag_in_scaled_c / l_inf_channel.cpu().numpy())
        return delta_im      
